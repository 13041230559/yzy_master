...
public class MultyPicView extends ViewGroup {
    ...
    private OnLongClickListener mLongClickListener = new OnLongClickListener() {
		...
		public boolean onLongClick(View v) {
			 if (v instanceof ImageView) {
	                Integer index = (Integer) v.getTag();
	                if (mClickCallback != null) {
	                    mClickCallback.onLongCall(index, mImgUrls);
	                }
	            }
    ...
    public MultyPicView(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (attr == R.styleable.multy_pic_view_pic_space) {
                ...
                mPicSpace = DisplayUtil.dip2px(mul);
            }
        }
        mSingleExpectMaxViewSize = Math.min(DisplayUtil.getScreenHeight((Activity) getContext()),
                DisplayUtil.getScreenWidth((Activity) getContext()));
    ...
    public int  test(int srcWidth , int srcHeight){
    	 int maxSize = Math.max(srcWidth, srcHeight);
        float ratio  = 1f;
         if (maxSize > mSingleExpectMaxViewSize) {
        	 ratio = maxSize * 1.0f / mSingleExpectMaxViewSize;
         }
        return  (int) (srcWidth / ratio) ;
    }
    
    public ImageView getImageView(int index){
    	return (ImageView) this.getChildAt(index);
    }
    ...
    public interface ClickCallback {
        ...
        void onLongCall(int index, String[] str);
    }
...
//                    left = (getMeasuredWidth() - mSingleSrcWidth) / 2;// 居中
    ...
    private void dealWithImgs(String[] imgs) {
        ...
        for (int i = 0; i < maxChildCount; i++) {
            ...
            if (i < imgLen) {
                ...
                chileIv.setAdjustViewBounds(true);
                chileIv.setMaxHeight(DisplayUtil.dip2px(150));
                chileIv.setMaxWidth(DisplayUtil.dip2px(150));
                String url = imgs[i];
                //GildeUtils.showPicThumb(context,url,chileIv);
                Glide.with(context).load(url).apply(new RequestOptions().centerCrop().error(R.mipmap.ic_launcher)).into(chileIv);
            } else {
    ...
    Drawable drawable = getContext().getResources().getDrawable(R.color.main_color);
