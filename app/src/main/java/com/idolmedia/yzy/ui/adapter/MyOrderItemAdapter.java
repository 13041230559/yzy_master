...
public class MyOrderItemAdapter extends BaseQuickAdapter<MyOrderEntity.DatasBean.OrderItemsBean.CatalogItemsBean,BaseViewHolder> {
    public MyOrderItemAdapter() {
        super(R.layout.order_detalis_item);
    }

    @Override
    protected void convert(BaseViewHolder helper, MyOrderEntity.DatasBean.OrderItemsBean.CatalogItemsBean item) {
        Glide.with(mContext).load(item.getCatalog_img()).apply(new RequestOptions().centerCrop().error(R.mipmap.default_bg)).into((ImageView) helper.getView(R.id.iamge_mainfigure));
